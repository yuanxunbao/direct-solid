...
# NOTE: for numbers entered here, if having units: length in micron, time in second, temperature in K.
    ...
    W0 = 20*d0                    # interface thickness      um
...
def simu_para(W0,Dl_tilde):
    ...
    lx = 10/W0                         # horizontal length in units of W0
    dx = 0.8
    nx = np.floor(lx/dx)
    dt = 0.8*(dx)**2/(4*Dl_tilde)       # time step size for forward euler
    Mt = 50000                     	# total  number of time steps
    ...
    ictype = 3                 	# initial condtion: 0 for semi-circular, 1 for planar interface, 2 for sum of sines
...
def sum_sine_initial(lx,nx,xx,zz,z0): 
    ...
    for kk in range(k_max):
        ...
        sp = sp + A[kk]*np.cos(2*math.pi*(kk+1)/lx* (xx-x_c[kk]) );
