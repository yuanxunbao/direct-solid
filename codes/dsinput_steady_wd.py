...
def phys_para(macrodata, tid, gid):    
# NOTE: for numbers entered here, if having units: length in micron, time in second, temperature in K.
    ...
    G0 = macroGR['Gt'][tid,gid]; Gt=np.array([G0,G0])
    R0 = macroGR['Rt'][tid,gid]; Rt=np.array([R0,R0])
    t_macro = macroGR['t_macro']
    d_macro = macroGR['d_macro']
    ...
    W0 = 100e-3                    # interface thickness      um
    ...
    return delta, k, lamd, Dl_tilde, W0, tau0, c_inf, m,  Ti, U_0, Gt, Rt, t_macro, macrodata[8:-4], angle, tid, gid
...
def simu_para(W0,Dl_tilde, tend):
    ...
    lx = 60/W0  #18.1*2.5/W0                         # horizontal length in units of W0
    dx = 0.8
    nx = np.floor(lx/dx)
    ...
    ictype = 1                 	# initial condtion: 0 for semi-circular, 1 for planar interface, 2 for sum of sines, 4 for transient data

    direc = '/work/07429/yxbao/frontera/steadyrun/'  #'/scratch/07428/ygqin/weld_line/'                	# direc = '/scratch/07429/yxbao/data'    # saving directory
