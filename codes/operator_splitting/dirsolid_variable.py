#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 25 23:10:13 2020

@author: yigong qin, yuanxun bao
"""
import importlib
import sys
import os
from scipy.io import savemat as save
from numba import njit, stencil, vectorize, float32, float64
import numpy as np
import math
from numpy.random import random
import time
import copy
from scipy import sparse as sp
from scipy.sparse import linalg as spla
import pyamgx
PARA = importlib.import_module(sys.argv[1])
#import dsinput as PARA

delta, k, lamd, R_tilde, Dl_tilde, lT_tilde, W0, tau0, c_infty, G, R = PARA.phys_para()
eps, alpha0, lxd, aratio, nx, dt, Mt, eta, seed, U_0, nts, filename, direc, \
mvf, Nset, ictype  = PARA.simu_para(W0,Dl_tilde)

k=1
alpha0 = alpha0*math.pi/180

cosa = np.cos(alpha0)
sina = np.sin(alpha0)

a_s = 1 - 3*delta
epsilon = 4*delta/a_s
a_12 = 4*a_s*epsilon
sqrt2 = np.sqrt(2.)

lx = lxd/W0
lz = aratio*lx

nz = int(aratio*nx+1)
nx = nx+1
nv= nz*nx #number of variables
dx = lx/(nx-1)
dz = lz/(nz-1)
print("mesh dx,dz",dx,dz)

x = np.linspace(0,lx,nx)
z = np.linspace(0,lz,nz)

zz,xx = np.meshgrid(z,x)
t=0

dxdz_in = 1./(dx*dz)  
dxdz_in_sqrt = np.sqrt(dxdz_in)

hi= 1./dx
dt_sr = np.sqrt(dt)


order_param = np.zeros((nv,nts+1))
conc = np.zeros((nv,nts+1))

np.random.seed(seed)

#Nset = nz - 81 #int(7/8*nz)
Ntip=1
Ntip_arr= np.zeros(Mt)
ztip_arr= np.zeros(Mt)
sp_form='csr'

pyamgx.initialize()
cfg = pyamgx.Config().create_from_dict({
   "config_version": 2,
        "determinism_flag": 1,
        "exception_handling" : 1,
        "solver": {
            "monitor_residual": 1,
            "solver": "CG", #"BICGSTAB",
            "convergence": "RELATIVE_INI_CORE",
            "tolerance" : 1e-8,
            "preconditioner": {
                "solver": "NOSOLVER"
        }
    }
})
cfg2 = pyamgx.Config().create_from_dict({
   "config_version": 2,
        "determinism_flag": 1,
        "exception_handling" : 1,
        "solver": {
            "monitor_residual": 1,
            "solver": "FGMRES", #"BICGSTAB",
            "convergence": "RELATIVE_INI_CORE",
            "tolerance" : 1e-8,
            "preconditioner": {
                "solver": "AMG"
        }
    }
})
rsc = pyamgx.Resources().create_simple(cfg)
rsc2 = pyamgx.Resources().create_simple(cfg2)
# rsc is resources would be used, pyamgx only supports single gpu
# Create matrices and vectors on gpu memory for amgx calls:
A = pyamgx.Matrix().create(rsc, mode ='dDDI')
B = pyamgx.Matrix().create(rsc, mode ='dDDI')
b = pyamgx.Vector().create(rsc, mode ='dDDI') # default on gpu memory
x = pyamgx.Vector().create(rsc, mode ='dDDI')

# Create solver:
solver = pyamgx.Solver().create(rsc, cfg)
solver2 = pyamgx.Solver().create(rsc2, cfg2)

def amgx(sol, rhs):
  # sol and rhs here are cpu arrays, needs to upload
  # for gpu arrays sol and rhs, use vec.upload_raw(d_a.device_ctypes_pointer.value, size)
  b.upload(rhs)
  x.upload(sol)

  # Setup and solve system:
  #solver.setup(A)
  # here, check if initial guess plays a role
  solver.solve(b, x)#, zero_initial_guess=True)

  # Download solution
  x.download(sol)

def amgx_var_coeff(sol, rhs, Mat):

  b.upload(rhs)
  x.upload(sol)
  B.replace_coefficients(Mat)
  solver2.setup(B)
  solver2.solve(b, x)#, zero_initial_guess=True)

  # Download solution
  x.download(sol) 


def sparse_cg(A, b, u0, TOL, P, maxit):

      num_iters = 0

      def callback(xk):
         nonlocal num_iters
         num_iters+=1

      x,status = spla.cg(A, b, x0=u0, tol=TOL, M=P, maxiter = maxit, callback=callback)
      return x,status,num_iters

def sparse_laplacian(nx,ny):
    
    # Neumann BC Laplacian
    Ix = sp.eye(nx,format=sp_form); Iy = sp.eye(ny,format=sp_form)
    Ix[0,0] = 0.5; Ix[-1,-1] = 0.5
    Iy[0,0] = 0.5; Iy[-1,-1] = 0.5
    


    Dxx = sp.diags([1, -2, 1], [-1, 0, 1], shape=(nx, nx)).toarray()
    Dxx[0,1]=2; Dxx[-1,-2]=2

    Dyy = sp.diags([1, -2, 1], [-1, 0, 1], shape=(ny, ny)).toarray()
    Dyy[0,1]=2; Dyy[-1,-2]=2


    # L = sp.kronsum(Dyy,Dxx,format='csc')
    L = sp.kronsum(Dxx,Dyy,format=sp_form)

    Q = sp.kron(Ix,Iy,format=sp_form)
    # Q = sp.kron(Ix,Iy,format='csc')
    
    return L,Q

def sparse_var_coeffi_laplacian(ph, nx, ny):
    K = 1-ph
  
    K_left = 0.5*(K[:-2,1:-1]+K[1:-1,1:-1]).reshape((nv),order='F')
    K_right = 0.5*(K[2:,1:-1]+K[1:-1,1:-1]).reshape((nv),order='F')
    K_top = 0.5*(K[1:-1,2:]+K[1:-1,1:-1]).reshape((nv),order='F')
    K_bottom = 0.5*(K[1:-1,:-2]+K[1:-1,1:-1]).reshape((nv),order='F')
    
    
    K_mid = -(K_left + K_right + K_top + K_bottom)
    # 'F' is the order that first traverse nx then ny
    
    #arrays = np.vstack((K_bottom, K_left, -(K_left+K_right+K_bottom+K_top), K_right, K_top))
 
    # set bottom adn top BCs: 
        #easy one: only need to add the missing values
    
    K_top[:nx] += K_bottom[:nx]
    K_bottom[-nx:] += K_top[-nx:]
        # hard one: add and remove 
    K_right[::nx] += K_left[::nx]
    K_left[nx-1::nx] += K_right[nx-1::nx]
    
    K_left[::nx]=0
    K_right[nx-1::nx]=0
    
    diag = np.array([-nx, -1, 0, 1, nx])
    
    ## 
    diff= sp.diags([ np.hstack((K_bottom[nx:],K_bottom[:nx])), np.hstack((K_left[1:],K_left[0])),  \
                         K_mid, K_right, K_top],\
                         diag, shape=(nv,nv),format=sp_form)
    
    #diff_mean= np.mean(diff,axis=1)

    return diff


def export_mat(CFL, nx, nz):
    
    Lap,Q = sparse_laplacian(nx, nz)
    I = sp.eye(nx * nz,format=sp_form)
    
    A0 = Q @ ( I - CFL*Lap)

    # preconditioner
    M2 = spla.splu(A0)
    M_x = lambda x: M2.solve(x)
    M = spla.LinearOperator((nv,nv), M_x)

    return Lap, Q, I, A0, M



def move_frame(Ntip, psi, phi, U, zz):
    
    
    #phi_tip = phi[Ntip,:]
    phi_tip = phi[1:-1,Ntip]
    
    while np.mean(phi_tip)>-0.99:
        
        Ntip += 1
        
        #phi_tip = phi[Ntip,:]
        phi_tip = phi[1:-1,Ntip]
        
        
    if Ntip==Nset:
        # move everything down
        #flag = 1
        Ntip = Ntip -1
        U_new = U_0*np.ones((nx,1))
        zz_new = 2*zz[1:-1,[-2]] - zz[1:-1,[-3]]
        psi_new = 2*psi[1:-1,[-2]] - psi[1:-1,[-3]]
        
        U[1:-1,1:-1] = np.hstack( (U[1:-1,2:-1], U_new ))
        zz[1:-1,1:-1] = np.hstack(( zz[1:-1,2:-1], zz_new ))
        psi[1:-1,1:-1] = np.hstack(( psi[1:-1,2:-1], psi_new ))
   # else: 
        #flag =0
    
    return  psi, U, zz, Ntip, zz[3,Ntip]


@njit
def set_halo(u):
    
    m,n = u.shape
    ub = np.zeros((m+2,n+2))
    
    ub[1:-1,1:-1] = u
    
    return ub



@vectorize([float32(float32, float32),
            float64(float64, float64)])
def atheta(ux, uz):

    ux2 = ( cosa*ux + sina*uz )**2
    uz2 = ( -sina*ux + cosa*uz)**2
        
    # return MAG_sq2
    MAG_sq  = ux2 + uz2
    MAG_sq2 = MAG_sq**2
    
    if (MAG_sq > eps):
        
        return a_s*( 1 + epsilon*(ux2**2 + uz2**2) / MAG_sq2   )
        # return uz/MAG_sq2
    else:
        return 1.0
    
    
@vectorize([float32(float32, float32),
            float64(float64, float64)])
def aptheta(ux, uz):
    uxr = cosa*ux + sina*uz
    uzr = -sina*ux + cosa*uz
    ux2 = uxr**2
    uz2 = uzr**2
    
    MAG_sq  = ux2 + uz2
    MAG_sq2 = MAG_sq**2
    
    if (MAG_sq > eps):
        
        return -a_12*uxr*uzr*(ux2 - uz2) /  MAG_sq2
    
    else:
        return 0.0
    
    
# update global array, don't jit compile this function
def set_BC(u,BCx,BCy):
    
    # 0 periodic, 1 no flux (Neumann)
    
    if BCx == 0 :
                
        u[ 0,:] = u[-2,:] # 1st =  last - 1
        u[-1,:] = u[ 1,:] # last = second
        
    if BCx == 1 :
        
        u[ 0,:] = u[ 2,:] # 1st = 3rd
        u[-1,:] = u[-3,:] # last = last - 2
    
    if BCy == 0 :
        
        u[:, 0]  = u[:,-2]
        u[:,-1]  = u[:, 1]
        
    if BCy == 1 :
        
        u[:,0] = u[:,2]
        u[:,-1] = u[:,-3]
        
        
    #return u
        
@stencil
def _aniso(ph):
    phxn = ( ph[ 1, 0] - ph[-1, 0] ) * 0.5
    phzn = ( ph[ 0, 1] - ph[ 0,-1] ) * 0.5

    A = atheta(phxn,phzn)
    return A
    

@stencil
def _rhs_psi(A,ph,U,zz):

    # ps = psi, ph = phi
    
    # =============================================================
    # 
    # 1. ANISOTROPIC DIFFUSION
    # 
    # =============================================================
    
    # these ps's are defined on cell centers

    
    phipjp=( ph[ 1, 1] + ph[ 0, 1] + ph[ 0, 0] + ph[ 1, 0] ) * 0.25
    phipjm=( ph[ 1, 0] + ph[ 0, 0] + ph[ 0,-1] + ph[ 1,-1] ) * 0.25
    phimjp=( ph[ 0, 1] + ph[-1, 1] + ph[-1, 0] + ph[ 0, 0] ) * 0.25
    phimjm=( ph[ 0, 0] + ph[-1, 0] + ph[-1,-1] + ph[ 0,-1] ) * 0.25
    
    # ============================
    # right edge flux
    # ============================

    phx = ph[1,0]-ph[0,0]
    phz = phipjp - phipjm
    
    AR  = atheta( phx,phz)
    Ap = aptheta(phx,phz)
    JR = AR * (  - Ap*phz )
    
    # ============================
    # left edge flux
    # ============================

    phx = ph[0,0]-ph[-1,0]
    phz = phimjp - phimjm
    
    AL  = atheta( phx,phz)
    Ap = aptheta(phx,phz)
    JL = AL * (  - Ap*phz )
    
    # ============================
    # top edge flux
    # ============================

    phx = phipjp - phimjp
    phz = ph[0,1]-ph[0,0]


    AT  = atheta( phx,phz)
    Ap = aptheta(phx,phz)
    JT = AT * (  Ap*phx )
    
    # ============================
    # bottom edge flux
    # ============================

    phx = phipjm - phimjm
    phz = ph[0,0]-ph[0,-1]
    
    AB  = atheta( phx,phz)
    Ap = aptheta(phx,phz)
    JB = AB * (  Ap*phx )
    
    
    
    # =============================================================
    # 
    # 2. EXTRA TERM: sqrt2 * atheta**2 * phi * |grad psi|^2
    # 
    # =============================================================
    
    # d(phi)/dx  d(psi)/dx d(phi)/dz  d(psi)/dz at nodes (i,j)
    phxn = ( ph[ 1, 0] - ph[-1, 0] ) * 0.5
    phzn = ( ph[ 0, 1] - ph[ 0,-1] ) * 0.5

    #A = atheta(phxn,phzn)
    A2 = A[0,0]**2
   # gradps2 = (psxn)**2 + (pszn)**2
    extra = 2*A[0,0]*( (AR-AL)*phxn + (AT-AB)*phzn ) # -sqrt2 * A2 * ph[0,0] * gradps2
    

    # =============================================================
    # 
    # 3. double well (transformed): sqrt2 * phi + nonlinear terms
    # 
    # =============================================================
    
    Up = 0 #(zz[0,0] )/lT_tilde
    
    rhs_psi = ((JR-JL) + (JT-JB) + extra) * hi**2 + \
               - lamd* (1-ph[0,0]**2)**2 *(U[0,0] + Up) + \
                   ph[0,0] - ph[0,0]**3
        
    
    # =============================================================
    # 
    # 4. dpsi/dt term
    # 
    # =============================================================
    tp = (1-(1-k)*Up)
    tau_psi = tp*A2 if tp >= k else k*A2
    # update nonlinear term to the ph_new variable, save ph for U
    #ph_new[0,0] = ph[0,0] + dt*rhs_psi/tau_psi  
    # return rhs_psi/tau_psi + eta*(random()-0.5)/dt_sr*hi
    return rhs_psi/tau_psi 



@stencil
def _rhs_U(U,ph,phi_t):
    
    # define cell centered values
    phipjp=( ph[ 1, 1] + ph[ 0, 1] + ph[ 0, 0] + ph[ 1, 0] ) * 0.25
    phipjm=( ph[ 1, 0] + ph[ 0, 0] + ph[ 0,-1] + ph[ 1,-1] ) * 0.25
    phimjp=( ph[ 0, 1] + ph[-1, 1] + ph[-1, 0] + ph[ 0, 0] ) * 0.25
    phimjm=( ph[ 0, 0] + ph[-1, 0] + ph[-1,-1] + ph[ 0,-1] ) * 0.25

    # ============================
    # right edge flux (i+1/2, j)
    # ============================
    phx = ph[1,0]-ph[0,0]
    phz = phipjp - phipjm
    phn2 = phx**2 + phz**2
    nx = phx / np.sqrt(phn2) if (phn2 > eps) else 0.
    
    jat    = (1+(1-k)*U[0,0])*phi_t[0,0]/sqrt2
    jat_ip = (1+(1-k)*U[1,0])*phi_t[1,0]/sqrt2
        
    UR = hi*Dl_tilde*0.5*(2 - ph[0,0] - ph[1,0])*(U[1,0]-U[0,0]) + \
         0.5*(jat + jat_ip)*nx
    UR = hi*Dl_tilde*  (U[1,0]-U[0,0])   
         
    # ============================
    # left edge flux (i-1/2, j)
    # ============================
    phx = ph[0,0]-ph[-1,0]
    phz = phimjp - phimjm
    phn2 = phx**2 + phz**2
    nx = phx / np.sqrt(phn2) if (phn2 > eps) else 0.
    
    jat_im = (1+(1-k)*U[-1,0])*phi_t[-1,0]/sqrt2
    
    UL = hi*Dl_tilde*0.5*(2 - ph[0,0] - ph[-1,0])*(U[0,0]-U[-1,0]) + \
         0.5*(jat + jat_im)*nx
    UL = hi*Dl_tilde*  (U[0,0]-U[-1,0])   
         
    # ============================
    # top edge flux (i, j+1/2)
    # ============================     
    phx = phipjp - phimjp
    phz = ph[0,1]-ph[0,0]
    phn2 = phx**2 + phz**2
    nz = phz / np.sqrt(phn2) if (phn2 > eps) else 0.
          
    jat_jp = (1+(1-k)*U[0,1])*phi_t[0,1]/sqrt2      
    
    UT = hi*Dl_tilde*0.5*(2 - ph[0,0] - ph[0,1])*(U[0,1]-U[0,0]) + \
         0.5*(jat + jat_jp)*nz
    UT = hi*Dl_tilde*  (U[0,1]-U[0,0])     
         
    # ============================
    # top edge flux (i, j-1/2)
    # ============================  
    phx = phipjm - phimjm
    phz = ph[0,0]-ph[0,-1]
    phn2 = phx**2 + phz**2
    nz = phz / np.sqrt(phn2) if (phn2 > eps) else 0.
    
    jat_jm = (1+(1-k)*U[0,-1])*phi_t[0,-1]/sqrt2      
    
    UB = hi*Dl_tilde*0.5*(2 - ph[0,0] - ph[0,-1])*(U[0,0]-U[0,-1]) + \
         0.5*(jat + jat_jm)*nz
    UB = hi*Dl_tilde*  (U[0,0]-U[0,-1])
    rhs_U = ( (UR-UL) + (UT-UB) ) * hi + sqrt2 * jat
    tau_U = (1+k) - (1-k)*ph[0,0]
    
    
    return rhs_U/tau_U
    

@njit(parallel=True)
def rhs_phi(ph_new,ph,U,zz): return _rhs_psi(ph_new,ph,U,zz)

@njit(parallel=True)
def aniso(ph): return _aniso(ph)

@njit(parallel=True)
def rhs_U(U,ph,phi_t): return _rhs_U(U,ph,phi_t)



def save_data(phi,U):

    cr =  1+ (1-k)*U_0
    c_tilde = ( 1+ (1-k)*U )*( k*(1+phi)/2 + (1-phi)/2 )/cr
    # c_tilde = ( 1+ (1-k)*U )*( k*(1+phi)/2 + (1-phi)/2 )

    return np.reshape(phi[1:-1,1:-1],     (nv,1), order='F') , \
           np.reshape(c_tilde[1:-1,1:-1], (nv,1), order='F')



if ictype == 0:

    psi0 = PARA.seed_initial(xx,lx,zz)
    phi0 = np.tanh(psi0/sqrt2)
    U0 = U_0*(phi0<=0)  ## for specific case in the paper

elif ictype == 1:

    psi0 = PARA.planar_initial(lz,zz)
    U0 = 0*psi0 + U_0
    phi0 = np.tanh(psi0/sqrt2)

elif ictype == 2:

    psi0 = PARA.sum_sine_initial(lx,nx,xx,zz)
    U0 = 0*psi0 + U_0
    phi0 = np.tanh(psi0/sqrt2)

else:
    print('ERROR: invalid type of initial condtion...' )
    sys.exit(1)

# append halo
psi = set_halo(psi0)
U = set_halo(U0)
zz = set_halo(zz)

set_BC(psi, 1, 1)
phi = np.tanh(psi/sqrt2)   # expensive replace
set_BC(U, 1, 1)
order_param[:,[0]], conc[:,[0]] = save_data(phi,U)

phi_old = copy.deepcopy(phi)

# For all numba routines to JIT-compile
start = time.time()
dPSI = rhs_phi(phi, phi_old, U, zz)
set_BC(dPSI, 1, 1)
#dU = rhs_U(U,phi,dPSI)
end = time.time()

print('compile time: ', end - start )

print(psi.shape)

kts = int(Mt/nts)

CFL = dt*hi**2  #CFL number
CFL2 = Dl_tilde*CFL/2

Lap, Q, I, A0, M = export_mat(CFL,nx,nz)

time9=0;time8=0;time7=0;time_reshape=0
time1=0;time2=0;time3=0;time4=0;time5=0;time6=0;time0=0;

# Upload system:

#M = sparse.csr_matrix(np.random.rand(5, 5))
#rhs = np.random.rand(5)
#sol = np.zeros(5, dtype=np.float64)
unew = np.zeros((nv)); phi_new = np.zeros((nv));
A.upload_CSR(A0)
B.upload_CSR(I-CFL2*sparse_var_coeffi_laplacian(phi, nx, nz))
solver.setup(A)
start = time.time()

for ii in range(Mt):

#==========================================================

                   # phi kernels    

#==========================================================
# step 1: explicit euler of nonlinear term (for inside)
    clock7 = time.time()
    Ani = aniso(phi_old)
    phi += dt* rhs_phi(Ani, phi_old, U, zz - R_tilde*t)
    clock0 = time.time(); time0+=clock0-clock7
    #dPSI = set_BC(dPSI, 1, 1)    
    #beta = random(psi.shape) - 0.5    
    #phi = phi + dt*dPSI #+ dt_sr*dxdz_in_sqrt*beta*eta
# step2: set BC for phi and A2(for halos)
    set_BC(phi, 1, 1)
    set_BC(Ani, 1, 1)
    clock1 = time.time();time1+=clock1-clock0
# step3: implicit heat kernel (no flux BCs)
    u = np.reshape(phi[1:-1,1:-1],     (nv,1), order='F')
    clock8= time.time();time7+= clock8-clock1 
    #unew,stat,num_iter = sparse_cg(A0, Q@u, u, 1e-8, M, 80)
    amgx(phi_new, Q@u)
    clock9= time.time();time8+= clock9-clock8
    #print("phi heat solve", num_iter)
    phi[1:-1,1:-1] = np.reshape(phi_new, (nx, nz), order='F')
    set_BC(phi, 1, 1)
    clock2 = time.time(); time2 +=clock2-clock1
# step4: analytic solution update

    phi = phi/np.sqrt(phi**2+ (1-phi**2)*np.exp(-2*dt/Ani**2))
    clock3 = time.time(); time3 +=clock3-clock2
#==========================================================

                   # U kernels    

#==========================================================
# step 5: explicit update of phi coupling
    #U = U + dt*rhs_U(U,phi,dPSI)
    #U = set_BC(U, 1, 1)    
    U += 0.5*(phi-phi_old)
    clock4 = time.time(); time4 +=clock4-clock3
# step 6: implicit heat kernel with constant diffusion D(no flux)
    u = np.reshape(U[1:-1,1:-1],     (nv,1), order='F')
    #unew,stat,num_iter = sparse_cg(A0D, Q@u, u, 1e-8, MD, 80)
    amgx_var_coeff(unew, u, I-CFL2*sparse_var_coeffi_laplacian(phi, nx, nz))
    #print("U heat solve", num_iter)
    U[1:-1,1:-1] = np.reshape(unew, (nx, nz), order='F')
    set_BC(U, 1, 1)
    clock5 = time.time(); time5+=clock5-clock4

    # =================================================================
    # If moving frame flag is set to TRUE
    # =================================================================
    if mvf == True :
    # check cross Nset, if true, move down
        psi, U, zz, Ntip,ztip = move_frame(Ntip, psi, phi, U, zz)
        Ntip_arr[ii]=Ntip; ztip_arr[ii]=ztip
    
    # add boundary
    #phi = set_BC(phi, 1, 1)
    #U = set_BC(U, 1, 1)
    
 
    # update phi
    #phi = np.tanh(psi/sqrt2) 
# step 7: update phi_old 
    t += dt
    phi_old = copy.deepcopy(phi)
    clock6 = time.time(); time6 += clock6-clock5

    if (ii+1)%kts==0:     # data saving 
       
        print('time step = ', (ii+1) )
        if mvf == True: print('tip position nz = ', Ntip)
         
        kk = int(np.floor((ii+1)/kts))
        order_param[:,[kk]], conc[:,[kk]] = save_data(phi,U)
    
end = time.time()

# Clean up:
A.destroy()
B.destroy()
x.destroy()
b.destroy()
solver.destroy()
rsc.destroy()
cfg.destroy()

pyamgx.finalize()

print('elapsed: ', end - start )
print(time0,time1,time2,time3, time4, time5, time6)
print('reshape and cg', time7,time8,(time2-time7-time8))
Uf = U[1:-1,1:-1]

save(os.path.join(direc,filename),{'xx':xx*W0,'zz':zz[1:-1,1:-1]*W0,'order_param':order_param,'conc':conc,'Ntip':Ntip_arr,'ztip':ztip_arr,'dt':dt*tau0,'nx':nx,'nz':nz,'Tend':t*tau0,'walltime':end-start,'dPSI':dPSI})
