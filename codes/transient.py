...
filename = 'dirsolid_trans_' + ext_name  + '_noise'+ \
...
# save initial data
order_param[:,[0]], conc[:,[0]], zz_mv[:,0], Uc[:,[0]] = save_data(psi_cpu, phi_cpu, U_cpu, z_cpu )
t_snapshot[0] = 0.
...
for kt in range(int(Mt/2)):
    ...
    if (2*kt+2)%kts==0:
       ...
       order_param[:,[kk]], conc[:,[kk]], zz_mv[:,kk], Uc[:,[kk]] = save_data(psi,phi,U,z_cpu)
...
save(os.path.join(direc,filename+'.mat'),{'order_param':order_param, 'conc':conc, 'Uc':Uc, 'xx':xx*W0, 'zz_mv':zz_mv*W0,'dt':dt*tau0,\
