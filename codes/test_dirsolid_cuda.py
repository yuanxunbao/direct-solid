...
# -*- coding: utf-8 -*-
"""
Created on Sat Jul  4 10:27:30 2020
...
@cuda.jit
def atheta_gpu(ux,uy,out):
    
    
    x, y = cuda.grid(2)
    if x < ux.shape[0] and y < uy.shape[1]:
       out[x,y] =atheta(ux,uy)
...
phi_cp = cp.asarray(phi)
out_cp = cp.empty_like(phi_cp)
...
for jj in range(nts):
    ...
    for ii in range(int(Mt/nts)):
        ...
        atheta_gpu(phi_cp,phi_cp, out_cp)
