...
#### define liquid to solid transition threshold ! #####
l2s = -0.995


mph = 'cell' if eta ==0.0 else 'dendrite'

filename = 'DNSlabel15'+ '_noise'+ \
...
@cuda.jit
def setNBC_gpu(ps,ph,U,dpsi,ph2,alpha, px, py, nprocx, nprocy, ha_wd):
    ...
    if ( px==0 and i<n):
        ...
        alpha[ha_wd-1,i]   = alpha[ha_wd+1,i]
    # no flux  at z = 0, lz
    if ( py==0 and i<m):
        ...
        alpha[i,ha_wd-1]   = alpha[i,ha_wd+1]
    if ( py==nprocy-1 and i<m):
        ...
        alpha[i,-ha_wd]   = alpha[i,-ha_wd-2]

@cuda.jit
def BC_421(ps, ph, U, dpsi, ph2, alpha, BC): # size of ph is nx+2*ny+2, ph_BC is 2(nx+ny), ha_wd
    ...
    if (  i< ny and j < ha_wd ):
        ...
        BC[i,j+5*ha_wd] = alpha[j+ha_wd,i+ha_wd]
        ...
        BC[i+ny,j+5*ha_wd] = alpha[j+nx,i+ha_wd]
    ...
    if (  i < nx and j < ha_wd ):
        ...
        BC[i+2*ny,j+5*ha_wd] = alpha[i+ha_wd,j+ha_wd]
        ...
        BC[i+2*ny+nx,j+5*ha_wd] = alpha[i+ha_wd,j+ny]


@cuda.jit
def BC_124(ps, ph, U, dpsi, ph2, alpha, BC): # size of ph is nx*ny, nbh_BC is 2(nx+ny), ha_wd
    ...
    if (  i< ny and j < ha_wd ):
        ...
        alpha[j,i+ha_wd]     = BC[i,j+5*ha_wd]
        ...
        alpha[j+nx+ha_wd,i+ha_wd]   = BC[i+ny,j+5*ha_wd]
    ...
    if (  i < nx and j < ha_wd ):
        ...
        alpha[i+ha_wd,j]   = BC[i+2*ny,j+5*ha_wd]
        ...
        alpha[i+ha_wd,j+ny+ha_wd] = BC[i+2*ny+nx,j+5*ha_wd]
   # the order of recv is [0,0], [-1,0], [0,-1], [-1,-1]
    if i < ha_wd and j < ha_wd:
       ...
       alpha[i,j]  =     BC[i+2*ny+2*nx+0*ha_wd,j+5*ha_wd];
       ...
       alpha[i+nx+ha_wd,j] =   BC[i+2*ny+2*nx+1*ha_wd,j+5*ha_wd];
       ...
       alpha[i,j+ny+ha_wd] =   BC[i+2*ny+2*nx+2*ha_wd,j+5*ha_wd];
       ...
       alpha[i+nx+ha_wd,j+ny+ha_wd] = BC[i+2*ny+2*nx+3*ha_wd,j+5*ha_wd];
...
@cuda.jit
def setBC_gpu(ps,ph,U,dpsi,alpha):
...
@cuda.jit
def XYT_lin_interp(x, y, t, X, Y, T,  u_3d,u_m ):
...
if ictype == 0: 
  ...
  #  alpha0 = (np.zeros_like(U0))*(phi0>0)
    alpha0 = (np.zeros_like(U0)+15*pi/180)*(phi0>l2s)
...
alpha_3D_gpu = cuda.to_device(alpha_cpu)
...
BCsend = cuda.device_array([2*nx+2*nz,6*ha_wd],dtype=np.float64);
BCrecv = cuda.device_array([2*nx+2*nz+4*ha_wd,6*ha_wd],dtype=np.float64)
#bpgBC = (2*bpg_x+2*bpg_y,1)
bpgBC = (2*(bpg_x+bpg_y),math.ceil(6*ha_wd/tpb) )
BC_421[bpgBC,tpb2d](psi_old,phi_old,U_old, dPSI, phi_old, alpha_m, BCsend)
#if rank == 0: print('sendbuf',BCsend.copy_to_host())
comm.Barrier()
BC_comm(BCsend, BCrecv, nx ,nz, 0)
comm.Barrier()
BC_124[bpgBC,tpb2d](psi_old,phi_old,U_old, dPSI, phi_old, alpha_m, BCrecv)
BC_124[bpgBC,tpb2d](psi_new,phi_new,U_new, dPSI, phi_new, alpha_m, BCrecv)
#if rank == 0: print('recvbuf',BCrecv.copy_to_host())
setNBC_gpu[bpg,tpb](psi_old,phi_old,U_new,dPSI, phi_old, alpha_m, px, py, nprocx, nprocy, ha_wd)
setNBC_gpu[bpg,tpb](psi_new,phi_new,U_old,dPSI, phi_new, alpha_m, px, py, nprocx, nprocy, ha_wd)
...
for kt in range(int(Mt/2)):
    ...
    # =================================================================
    t_cur = (2*kt)*dt*tau0
    XYT_lin_interp[bpg2d, tpb2d](x_gpu, z_gpu, t_cur, X_gpu, Z_gpu, mac_t_gpu, T_3D_gpu, T_m )
    rhs_psi[bpg2d, tpb2d](psi_old, phi_old, U_old, psi_new, phi_new, U_new, z_gpu, dPSI, 2*kt, rng_states, T_m, alpha_m)
    #if ha_wd==1:
    if (2*kt+2)%ha_wd==0:
      BC_421[bpgBC,tpb2d](psi_new, phi_new, U_old, dPSI,phi_old, alpha_m, BCsend)
      comm.Barrier()
      BC_comm(BCsend, BCrecv, nx ,nz,2*kt+1)
      comm.Barrier()
      BC_124[bpgBC,tpb2d](psi_new, phi_new, U_old, dPSI,phi_old, alpha_m, BCrecv)
    setNBC_gpu[bpg,tpb](psi_new,phi_new,U_old,dPSI, phi_old, alpha_m, px, py, nprocx, nprocy, ha_wd)
    ...
    if (2*kt+2)%ha_wd==0: #ha_wd==1 or ha_wd==2:
      BC_421[bpgBC,tpb2d](psi_old,phi_old,U_new, dPSI, phi_new, alpha_m, BCsend)
      comm.Barrier()
      BC_comm(BCsend, BCrecv, nx ,nz,2*kt+2)
      comm.Barrier()
      BC_124[bpgBC,tpb2d](psi_old,phi_old,U_new, dPSI, phi_new, alpha_m, BCrecv)
    setNBC_gpu[bpg,tpb](psi_old,phi_old,U_new,dPSI, phi_new, alpha_m, px, py, nprocx, nprocy, ha_wd)
